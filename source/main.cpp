...
int main() {
    ...
    snprintf(buf, 255, "https://dweet.io/follow/ep_agora_%lu", device_address);
    printf("Device Dweet Address: %s\r\n", buf);

    //dummy temp variable to be replaced with Atlas temp sensor fxn
    uint32_t retry_seconds = 2;
    NetworkInterface *network = nullptr;
    while(true) {
        network = connect_to_default_network_interface();
        if (!network) {
            printf("Could not connect to the network, see serial output. Retrying in %d seconds...\n",
                    retry_seconds);
    ...
    while (1) {
        ...
        {
            snprintf(buf, 255, "https://dweet.io:443/dweet/for/ep_agora_%lu",
                    device_address);
